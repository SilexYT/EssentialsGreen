...
public class give implements CommandExecutor {
	...
	public boolean onCommand(CommandSender p, Command cmd, String Label, String[] args){
		if(p.hasPermission("EssentialsGreen.give")){
			if(args.length == 0){
				p.sendMessage(main.prefix + "/give <Player> <Materiel | ID> [Ammount]");
			}else if(args.length == 1){
				p.sendMessage(main.prefix + "/give " + args[0] + " <ID> [Ammount]");
			}else if(args.length > 1){
				...
				if(!(target == null)){
					if(NumberManager.firstNumberCheck(args[1])){
						if(ItemManager.CheckID(new Integer(args[1])) == true){
							if(args.length == 2){
								...
								p.sendMessage(main.prefix + "Give " + target.getName() + " " + args[1] + " (" + Materiel + ") Ammount : 1");
							}else{
								Material Materiel = ItemManager.getMaterialByID(new Integer(args[1]));
								target.getInventory().addItem(new ItemStack(Materiel, new Integer(args[2])));
								p.sendMessage(main.prefix + "Give " + target.getName() + " " + args[1] + " (" + Materiel + ") Ammount : " + args[2]);
							}
						}else p.sendMessage(main.prefix + "ID not Found");
					...
					}else p.sendMessage(main.prefix + "Materiel not Found");
